...
<div class="login-overlay" id="loginOverlay">
  <div class="login-card">
    ...
    <p id="recoveryHint" style="display:none;font-size:12px;color:var(--text-secondary);margin-top:8px;background:var(--bg);padding:12px 14px;border-radius:6px;text-align:left;line-height:1.7">忘记密码后，使用 <strong>环境变量 <code>RESET_KEY</code></strong>、<strong><code>config.json</code> 中的 <code>resetKey</code></strong> 或 <strong>恢复码</strong> 的值作为密码登录即可恢复。<br><br>· 本地/Docker：查看 <code>config/config.json</code> 的 <code>resetKey</code> 字段<br>· Cloudflare：查看 Dashboard → Settings → Variables 中的 <code>RESET_KEY</code><br>· 恢复码：登录后在后台查看<br><br>默认值与管理员密码相同。</p>
  </div>
</div>
...
(function(){
  ...
  changePwdBtn.addEventListener('click',async()=>{
    ...
    try{
      ...
      toast('密码已修改');
      alert('密码已修改。如忘记新密码，可用环境变量 RESET_KEY 或恢复码恢复登录。');
