...
<div class="login-overlay" id="loginOverlay">
  <div class="login-card">
    ...
    <p id="recoveryHint" style="display:none;font-size:12px;color:var(--text-secondary);margin-top:8px;background:var(--bg);padding:12px 14px;border-radius:6px;text-align:left;line-height:1.7">忘记密码后，使用<strong>重置密钥（RESET_KEY）</strong>作为密码登录即可重置为 admin。<br><br>· 本地/Docker：查看 <code>config/config.json</code> 的 <code>resetKey</code> 字段<br>· Cloudflare：查看 Dashboard → Variables 中的 <code>RESET_KEY</code><br><br>密码重置后请及时修改密码。恢复码亦可在登录后后台查看。</p>
  </div>
</div>
...
<div class="pwd-card">
  ...
  <p style="font-size:12px;color:var(--text-secondary);margin:8px 0">恢复码可一次性替代管理员密码登录，用完即作废。也可在 <code>config/config.json</code>（本地）或 Cloudflare Dashboard Variables（Cloudflare）中查看。</p>
...
(function(){
  ...
  changePwdBtn.addEventListener('click',async()=>{
    ...
    try{
      ...
      toast('密码已修改');
      alert('密码已修改。如忘记新密码，可用 RESET_KEY 或恢复码恢复登录。');
  ...
  async function loadRecoveryCodes(){
    try{
      ...
      if(data.codes.length>0){
        rcList.innerHTML=data.codes.map(c=>\`<div style="padding:2px 0">· \${c}</div>\`).join('');
